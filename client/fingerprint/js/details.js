...
var language_codes = [
    ...
    [
        2476, 2494, 2434, 2482, 2494, 32, 47, 32, 2437, 2488, 2478, 2496, 2479,
        2492, 2494,
    ],
    [20206, 21517],
...
function getGPUTable(hashes) {
    ...
    for (hash in hashes) {
        if (hash >= 1) checked = "";
        res +=
            "<td><label width = '30px'><input id='box_" +
            hash +
            "' type='checkbox' " +
            checked +
            " onclick='gen_code();'/> " +
...
function getTable(name) {
    ...
    if (name == "langsdetected")
        return getLangsString(trans_data[name].split("_"));
...
function buildTable(data) {
    ...
    if (data["cpu_cores"] == -1 || data["cpu_cores"] == "-1") {
        //here we can also use == '-1'
        data["cpu_cores"] = "N/A";
    }
    ...
    $("#result_table").append(
        '<tr><td class = "checkbox"></td><td>Feature</td><td class = "value">Value</td></tr>'
    );
    ...
    for (var cross in cross_list) {
        if (cross in data) {
            ...
            $("#result_table").append(
                '<tr><td class = "checkbox"> <input id="box_' +
                    cross +
                    '"type="checkbox" onclick="gen_code();" checked></td><td class = "feature">' +
                    cross_list[cross] +
                    '</td><td class = "value">' +
                    value +
                    "</td></tr>"
    ...
    for (var feature in show_list) {
        ...
        if (feature in cross_list) continue;
        if (feature in data) {
            value = getTable(feature);
            $("#result_table").append(
                '<tr><td class = "checkbox"><input id="box_' +
                    feature +
                    '" type="checkbox" onclick="gen_code();"></td><td class = "feature">' +
                    show_list[feature] +
                    '</td><td class = "value">' +
                    value +
                    "</td></tr>"
...
function getDetails() {
    ip_address = "http://127.0.0.1:9875/";
